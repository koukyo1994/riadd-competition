...
class mAPCallback(Callback):
    def __init__(self, input_key: str = "targets", output_key: str = "logits", prefix="mAP"):
...
class MultiDiseaseAvgScore(Callback):
    ...
    def on_batch_end(self, runner: IRunner):
        ...
        y_true_map = targ[:, 1:].reshape(-1)
        y_pred_map = out[:, 1:].reshape(-1)

        auc_score = metrics.roc_auc_score(y_true=y_true_auc, y_score=y_pred_auc)
        map_score = metrics.average_precision_score(y_true=y_true_map, y_score=y_pred_map)
    ...
    def on_loader_end(self, runner: IRunner):
        ...
        y_true_map = y_true[:, 1:].reshape(-1)
        y_pred_map = y_pred[:, 1:].reshape(-1)
