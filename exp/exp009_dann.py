...
class TrainDataset(torchdata.Dataset):
    def __init__(self, df: pd.DataFrame, datadir: Path, target_columns: list, transform=None,
        ...
        self.domain_labels = df["camera"].values
...
class MultiDiseaseAvgScore(Callback):
    ...
    def on_batch_end(self, runner: IRunner):
        ...
        y_true = targ[:, 1:len(CFG.target_columns)]
        y_pred = out[:, 1:len(CFG.target_columns)]
...
class CompetitionScore(Callback):
    ...
    def on_batch_end(self, runner: IRunner):
        ...
        y_true_mdas = targ[:, 1:len(CFG.target_columns)]
        y_pred_mdas = out[:, 1:len(CFG.target_columns)]
    ...
    def on_loader_end(self, runner: IRunner):
        ...
        y_true_mdas = y_true[:, 1:len(CFG.target_columns)]
        y_pred_mdas = y_pred[:, 1:len(CFG.target_columns)]
