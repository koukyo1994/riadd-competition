...
class GradientReversalLayer(Function):
    ...
    def backward(ctx, grad):
        return grad.neg() * ctx.constant, None
...
class TimmModel(nn.Module):
    ...
    def init_layer(self):
        ...
        init_layer(self.domain_classifier)
...
class DomainAUCCallback(Callback):
    def __init__(self, input_key: str = "domain_targets", output_key: str = "logits", prefix="domain_auc"):
    ...
    def on_batch_end(self, runner: IRunner):
        ...
        out = runner.output[self.output_key].detach()[:, -1]
    ...
    if CFG.oof:
        ...
        for i, (_, val_idx) in enumerate(splitter.split(train, y=train[CFG.target_columns])):
            ...
            for batch in tqdm(val_loader, desc=f"fold{i} oof"):
                ...
                disease_pred = torch.sigmoid(output[:, :-1]).cpu().numpy()
                domain_pred = torch.sigmoid(output[:, -1]).cpu().numpy()
            ...
            domain_auc = metrics.roc_auc_score(y_true=y_true_domain, y_score=y_pred_domain)
            logger.info(f"Fold {i} Domain AUC: {domain_auc:.5f}")
        ...
        domain_auc = metrics.roc_auc_score(y_true=y_true_domain, y_score=y_pred_domain)
        logger.info(f"Fold {i} Domain Acc: {domain_auc:.5f}")
