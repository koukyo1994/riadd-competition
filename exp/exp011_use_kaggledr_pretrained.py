...
class CFG:
    ...
    pretrained_model = "./out/kaggle_dr_pretrain/fold-0.ckpt"
...
def prepare_model_from_pretrained(model, path: Path, model_state_dict_key="state_dict"):
    if not torch.cuda.is_available():
        ckpt = torch.load(path, map_location="cpu")
    else:
        ckpt = torch.load(path)
    ...
    model.base_model.classifier = nn.Linear(in_features, num_classes)
    return model
...
if __name__ == "__main__":
    ...
    if CFG.train:
        ...
        for i, (trn_idx, val_idx) in enumerate(splitter.split(train, y=train[CFG.target_columns])):
            ...
            model = prepare_model_from_pretrained(model, Path(CFG.pretrained_model))
