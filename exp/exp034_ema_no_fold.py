...
def prepare_ema_for_inference(model, path: Path):
    if not torch.cuda.is_available():
        ckpt = torch.load(path, map_location="cpu")
    else:
        ckpt = torch.load(path)
    ...
    for key in ckpt["model_state_dict"]:
        ...
        model_state_dict[new_key] = ckpt["model_state_dict"][key]
...
def calc_metrics(y_true: np.ndarray, y_pred: np.ndarray):
    auc = metrics.roc_auc_score(y_true[:, 0], y_pred[:, 0])
    mdas = multi_disease_avg_score(y_true[:, 1:], y_pred[:, 1:])
    return 0.5 * auc + 0.5 * mdas, auc, mdas
...
def train_func(model,
    ...
    for epoch in range(epochs):
        ...
        score, auc, mdas = calc_metrics(y_true, y_pred)
        train_metrics["loss"] = avg_loss
        train_metrics["score"] = score
        train_metrics["auc"] = auc
        train_metrics["MdAS"] = mdas
        ...
        eval_metrics["EMA_score"] = score
        eval_metrics["EMA_auc"] = auc
        eval_metrics["EMA_MdAS"] = mdas
...
if __name__ == "__main__":
    ...
    if CFG.train:
        ...
        loaders = {
            "train": torchdata.DataLoader(TrainDataset(
                    train,
                    CFG.train_datadir,
                    CFG.target_columns,
                    transform=get_transforms(CFG.img_size, "train")),
                **CFG.loader_params["train"]),  # type: ignore
            "valid": torchdata.DataLoader(TrainDataset(
                    train,
                    CFG.train_datadir,
                    CFG.target_columns,
                    transform=get_transforms(CFG.img_size, "vald")),
                **CFG.loader_params["valid"])  # type: ignore
            }
        model = TimmModel(
    ...
    if CFG.inference:
        ...
        for i in CFG.folds:
            ...
            model = TimmModel(
                ...
                pretrained=CFG.pretrained,
                num_classes=CFG.num_classes)
            model = prepare_ema_for_inference(model, logdir / "ema.pth").to(device)
            predictions = []
            ids = []
