...
class CFG_Risk(CFG):
    ...
    target_columns = ["Disease_Risk"]
    num_classes = 1
    loss_name = "BCEWithLogitsLoss"


class CFG_Disease(CFG):
...
def get_optimizer(model: nn.Module, cfg):
...
class SeparatedLoss(nn.Module):
    def __init__(self, loss_disease_risk="BCEWithLogitsLoss", loss_disease="BCEFocalLoss", weights=[1.0, 1.0]):
        super().__init__()
    ...
    def forward(self, preds, targets):
        ...
        return self.weights[0] * loss_disease_risk + self.weights[1] * loss_disease


__CRITERIONS__ = {
    "BCEFocalLoss": BCEFocalLoss,
    "SeparatedLoss": SeparatedLoss,
}
...
def get_callbacks(target="auc"):
    if target == "auc":
        callbacks = [
            AUCCallback(),
            CheckpointCallback(metrics_filename="auc_metrics.json"),
            CheckpointAtBestMetric("epoch_auc", is_higher_better=True)
        ]
    elif target == "MdAS":
        callbacks = [
            MultiDiseaseAvgScore(),
            CheckpointCallback(metrics_filename="MdAS_metrics.json"),
    ...
    if CFG.optimizer_name == "SAM":
        callbacks.append(SchedulerCallback())
    ...
    if CFG.train:
        ...
        for i, (trn_idx, val_idx) in enumerate(splitter.split(train, y=train[CFG.target_columns])):
            ...
            model_risk = TimmModel(
                ...
                pretrained=CFG_Risk.pretrained,
                num_classes=CFG_Risk.num_classes
            )
            criterion = get_criterion(CFG_Risk)
            optimizer = get_optimizer(model_risk, CFG_Risk)
            scheduler = get_scheduler(optimizer, CFG_Risk)
            callbacks = get_callbacks(target="auc")
            ...
            runner.train(
                ...
                optimizer=optimizer,
                scheduler=scheduler,
                num_epochs=CFG_Risk.epochs,
                verbose=True,
                logdir=logdir / f"fold{i}",
                callbacks=callbacks,
                main_metric=CFG_Risk.main_metric,
                minimize_metric=CFG_Risk.minimize_metric)

            del model_risk, optimizer, scheduler
            gc.collect()
            ...
            model_disease = TimmModel(
                ...
                pretrained=CFG_Disease.pretrained,
                num_classes=CFG_Disease.num_classes
            )
            criterion = get_criterion(CFG_Disease)
            optimizer = get_optimizer(model_disease, CFG_Disease)
            scheduler = get_scheduler(optimizer, CFG_Disease)
            callbacks = get_callbacks(target="MdAS")
            runner = get_runner(device)
            runner.train(
                model=model_disease,
                criterion=criterion,
                ...
                optimizer=optimizer,
                scheduler=scheduler,
                num_epochs=CFG_Disease.epochs,
                verbose=True,
                logdir=logdir / f"fold{i}",
                callbacks=callbacks,
                main_metric=CFG_Disease.main_metric,
                minimize_metric=CFG_Disease.minimize_metric)

            del model_disease, optimizer, scheduler
            gc.collect()
            torch.cuda.empty_cache()
    ...
    if CFG.oof:
        ...
        for i, (_, val_idx) in enumerate(splitter.split(train, y=train[CFG.target_columns])):
            ...
            for batch in tqdm(val_loader, desc=f"fold{i} oof"):
                ...
                output = np.zeros((len(output_auc), CFG.num_classes), dtype=np.float32)
                output[:, 0] = output_auc.reshape(-1)
                output[:, 1:] = output_mdas
