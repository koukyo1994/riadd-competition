...
class TrainDataset(torchdata.Dataset):
    def __init__(self, df: pd.DataFrame, datadir: Path, target_columns: list, transform=None, train=False):
        self.df = df
    ...
    def __getitem__(self, index: int):
        ...
        image = crop_image_from_gray(image, 20)
        if camera != "C2":
        ...
        if camera == "C1":
            ...
            image_size = int(320 * 3420 / 1389)
        else:
            image_size = int(320 * 1475 / 1389)
        if self.train:
            x_rand = np.random.randint(-30, 31)
            y_rand = np.random.randint(-30, 31)
        else:
            x_rand = 0
            y_rand = 0

        x_start = max(0, int(x - image_size // 2 + x_rand))
        x_end = x_start + image_size
        if x_end >= w:
            x_end = w - 1
            x_start = x_end - image_size

        y_start = max(0, int(y - image_size // 2 + y_rand))
        y_end = y_start + image_size
        if y_end >= h:
            y_end = h - 1
            y_start = y_end - image_size
...
class TestDataset(torchdata.Dataset):
    def __init__(self, df: pd.DataFrame, datadir: Path, transform=None):
        self.df = df
        self.filenames = df["ID"].values
        self.camera = df["camera"].values
        self.datadir = datadir
    ...
    def __getitem__(self, index: int):
        ...
        x_start = max(0, int(x - image_size // 2))
        x_end = x_start + image_size
        if x_end >= w:
            x_end = w - 1
            x_start = x_end - image_size

        y_start = max(0, int(y - image_size // 2))
        y_end = y_start + image_size
        if y_end >= h:
            y_end = h - 1
            y_start = y_end - image_size
...
if __name__ == "__main__":
    ...
    if CFG.oof:
        ...
        for i, (_, val_idx) in enumerate(splitter.split(train, y=train[CFG.target_columns])):
            ...
            y_true_mdas = targ_array
            y_pred_mdas = pred_array
            mdas_score = multi_disease_avg_score(y_true_mdas, y_pred_mdas)
        ...
        y_true_mdas = targ_df[CFG.target_columns].values
        y_pred_mdas = pred_df[CFG.target_columns].values
        mdas_score = multi_disease_avg_score(y_true_mdas, y_pred_mdas)
