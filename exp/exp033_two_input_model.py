...
class TrainDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        optic_disk = image[y_start:y_end, x_start:x_end]

        if self.transform:
            augmented = self.transform(image=image)
            image = augmented["image"]
            augmented = self.transform(image=optic_disk)
            optic_disk = augmented["image"]
...
class TimmModel(nn.Module):
    def __init__(self, base_model_name="tf_efficientnet_b0_ns", pooling="GeM", pretrained=True, num_classes=24):
        ...
        self.od_model = timm.create_model(base_model_name, pretrained=pretrained)
...
class SAMRunner(Runner):
    ...
    def _handle_batch(self, batch):
        ...
        input_od = batch["od_image"]

        input_ = input_.to(self.device)
        input_od = input_od.to(self.device)
        target = target.to(self.device)

        out = self.model(input_, input_od)
        ...
        if self.is_train_loader:
            ...
            self.criterion(self.model(input_, input_od), target).backward()
